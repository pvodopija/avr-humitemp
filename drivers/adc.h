...
#define adcENABLE() macsREG_BIT_SET( ADCSRA, ADEN, ON );    /* Turns on the ADC. An empty read should be performed since it initializes the ADC and lasts a bit longer ( 25 cycles instead of normal 13 ). */
#define adcDISABLE() macsREG_BIT_SET( ADCSRA, ADEN, OFF );  /* Turns off the ADC, should be used in Power-Saving modes. */
...
static uint16_t uiAdcRead( uint8_t uiAdcInput )
{
    ...
    macsREG_BIT_SET( ADCSRA, ADSC, ON );            /* Start conversion. */
...
static void vAdcInit()
{
        ...
        macsREG_BIT_SET( ADMUX, ADLAR, ON );
    #endif

    #if ( adcUSE_INTERNAL_REFERENCE == 1 )
        ADMUX |= 0b11000000;
    #else
        macsREG_BIT_SET( ADMUX, REFS0, ON );
    #endif
